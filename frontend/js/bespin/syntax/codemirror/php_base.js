...
dojo.declare("bespin.syntax.codemirror.PHPBase", bespin.syntax.codemirror.Base, {
    ...
    tokenizer: function(source, startState) {
        ...
        function phpToken(inside, source, setInside) {
            ...
            function readHeredoc(identifier){
                ...
                if (identifier == "<<<") {
                    ...
                    } else if (source.applies(self.matcher(/[A-Za-z_]/))) {
                        // heredoc
                        source.nextWhile(isWordChar);
                        identifier = source.get();
                    } else {
        ...
        return this.baseTokenizer.call(this, source, startState || phpTokenState(false, true));
    },
    ...
    PHPLexical: function(indented, column, type, align, prev, info) {
    ...
    makeParser: function(input, basecolumn) {
        ...
        var lexical = new this.PHPLexical((basecolumn || 0) - this.indentUnit, 0, "block", false);
        ...
        function next() {
            ...
            if (token.content == "\n") {
                ...
                // them.
                token.indentation = self.indentPHP.call(self, lexical);
            }
        ...
        function pushlex(type, info) {
            var result = function pushlexing() {
                lexical = new self.PHPLexical(indented, column, type, null, lexical, info);
            };
            result.lex = true;
            return result;
        }
        ...
        function statement(token){
            ...
            if (type == "keyword a") {
                ...
                cont(pushlex("form"), require("("), pushlex(")"), expression, require("as"), require("variable"), /* => $value */ expect(")"), poplex, statement, poplex);
            } else if (type == "for") {
                ...
                cont(require("t_string"));  // 'const static x=5' is a syntax error
        ...
        function expression(token){
            ...
            if (self.AtomicTypes.hasOwnProperty(type)) {
                ...
                cont(require("string"), maybeoperator);  // heredoc/nowdoc
        ...
        function maybeoperator(token){
            ...
            if (type == "operator") {
                if (token.content == "?") {
                    cont(expression, require(":"), expression);  // ternary operator
                } else {
                    cont(expression);
                }
            } else if (type == "(") {
                cont(pushlex(")"), expression, commasep(expression), require(")"), poplex, maybeoperator /* $varfunc() + 3 */);
            } else if (type == "[") {
                cont(pushlex("]"), expression, require("]"), maybeoperator /* for multidimensional arrays, or $func[$i]() */, poplex);
            }
        }
        ...
        function funcarg(token) {
            if (token.type == "t_string") {  // function foo(myclass $obj) {...}
                cont(require("variable"), maybedefaultparameter);
            } else if (token.type == "variable") {  // function foo($string) {...}
